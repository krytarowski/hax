    ...
    char **argv				/* Argument strings. */)
{
	...
	length = strlen(libraryPath) + strlen("/") + strlen("lib") +
		strlen(name) + strlen(reqVersion) + strlen(".so") + 1;
	dsoPath = ckalloc(memoryp, length);
	sprintf(dsoPath, "%s/lib%s%s.so", libraryPath, name, reqVersion);
	...
	Hax_Package *ctx;
	length = strlen("Hax_") + strlen(name) + strlen("_ctx") + 1;
	ctxName = ckalloc(memoryp, length);
	sprintf(ctxName, "Hax_%s_ctx", name);
	ctx = (Hax_Package *) dlsym(newDso->dsoPtr, ctxName);
	printf("major=%d minor=%d patch=%d name=%s version=%s\n", ctx->haxMajor, ctx->haxMinor, ctx->haxPatch, ctx->name, ctx->version);
	(*ctx->packageInit)(interp, clientData);
