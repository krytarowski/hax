...
#elif DBL_MANT_DIG == 64 && DBL_MAX_EXP == 16384
...
static long long Hax_scanexp(FILE *f, int pok)
{
	int c;
	int x;
	long long y;
...
static double Hax_decfloat(FILE *f, int c, int bits, int emin, int sign, int pok)
{
	uint32_t x[KMAX];
...
double Hax___floatscan(FILE *f, int prec, int pok)
{
	...
	if (c=='0') {
		c = shgetc(f);
		if ((c|32) == 'x')
			return Hax_hexfloat(f, bits, emin, sign, pok);
		shunget(f);
		c = '0';
	}
